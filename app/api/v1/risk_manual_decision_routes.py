...
def _get_task_user_id(risk_point, db: Session) -> Optional[str]:
    ...
    if isinstance(risk_point, RiskPoint):
        task = (
            db.query(ReviewTask)
            .filter(ReviewTask.id == risk_point.review_task_id)
            .first()
        )
        ...
        task = (
            ...
            .filter(ComparisonTask.id == risk_point.comparison_task_id)
            .first()
        )
        return task.user_id if task else None
    return None
...
@risk_manual_decision_router.patch(
    "/{risk_id}/status", response_model=RiskStatusUpdateResponse
)
async def update_risk_status(
    ...
    if not risk_point:
        ...
        risk_point = (
            ...
            .filter(ComparisonRiskPoint.id == risk_id)
            .first()
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"无效的状态值: {request.status}，必须是 pending/confirmed/ignored",
        )
