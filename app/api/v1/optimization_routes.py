...
from app.models.database import OptimizedContractVersion, ReviewTask, RiskPoint, User
...
@optimization_router.post("/reviews/{task_id}/suggestions/apply", response_model=ApplySuggestionsResponse)
async def apply_review_suggestions(
    ...
    try:
        version = create_optimized_version(db, task, risk_points, current_user.id, request.title)
        db.commit()
...
@optimization_router.get("/reviews/{task_id}/optimized-versions", response_model=list[OptimizedContractVersionResponse])
async def list_optimized_versions(
    ...
    versions = db.query(OptimizedContractVersion).filter(
        OptimizedContractVersion.review_task_id == task_id
    ).order_by(OptimizedContractVersion.version_no.desc()).all()
    return [OptimizedContractVersionResponse.model_validate(version.to_dict()) for version in versions]


@optimization_router.get("/reviews/{task_id}/optimized-versions/{version_id}", response_model=OptimizedContractVersionResponse)
...
def _get_optimized_version(db: Session, task_id: str, version_id: str) -> OptimizedContractVersion:
    """查询优化合同版本。"""
    version = db.query(OptimizedContractVersion).filter(
