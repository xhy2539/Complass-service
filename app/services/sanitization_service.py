...
SENSITIVE_PATTERNS = [
    ("company", "公司", re.compile(r"[\u4e00-\u9fa5]{2,50}(?:有限责任公司|科技有限公司|有限公司|集团|银行|公司)")),
    ...
    ("bank_name", "开户行", re.compile(r"开户行[:：]?\s*[\u4e00-\u9fa5]{2,40}(?:银行|支行|分行)")),
    ("address", "地址", re.compile(r"(?:地址|住所|注册地址)[:：]?\s*[\u4e00-\u9fa5A-Za-z0-9\-号弄室座层栋路街区县市省]{6,80}")),
]
...
def sanitize_contract_text(text: str) -> SanitizationResult:
    """脱敏合同文本，保留金额和日期。"""
    if not text:
        return SanitizationResult(sanitized_text="", mappings=[], errors=["合同文本为空，无法脱敏"])
    ...
    sanitized = _sanitize_person_names(sanitized, mappings, counters, original_to_placeholder)
...
def restore_text_from_mapping(text: str, mappings: list[dict]) -> str:
    """将脱敏占位符还原为原文。"""
    restored = text or ""
    for item in sorted(mappings or [], key=lambda value: len(value.get("placeholder", "")), reverse=True):
...
def apply_sanitization_mappings(text: str, mappings: list[dict]) -> str:
    ...
    result = text or ""
    for item in sorted(mappings or [], key=lambda value: len(value.get("original", "")), reverse=True):
        placeholder = item.get("placeholder")
        original = item.get("original")
        if placeholder and original:
...
def _sanitize_person_names(
    ...
    """脱敏带有明确标签的联系人姓名。"""
    pattern = re.compile(r"(?P<prefix>(?:联系人|法定代表人|授权代表|经办人|签署人|代表)[:：]?\s*)(?P<name>[\u4e00-\u9fa5]{2,4})")
